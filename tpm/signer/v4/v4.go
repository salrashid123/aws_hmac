...
import (
	...
	hmaccred "github.com/salrashid123/aws_hmac/tpm/signer"
)
...
type HTTPSigner interface {
	...
	SignHTTP(ctx context.Context, credentials hmaccred.TPMSigner, r *http.Request, payloadHash string, service string, region string, signingTime time.Time, optFns ...func(*SignerOptions)) error
}

type keyDerivator interface {
	DeriveKey(credential hmaccred.TPMSigner, service, region string, signingTime SigningTime) ([]byte, error)
}
...
type httpSigner struct {
	...
	Region       string
	Time         SigningTime
	Credentials  hmaccred.TPMSigner
	KeyDerivator keyDerivator
	IsPreSign    bool
...
func (s Signer) SignHTTP(ctx context.Context, credentials hmaccred.TPMSigner, r *http.Request, payloadHash string, service string, region string, signingTime time.Time, optFns ...func(options *SignerOptions)) error {
	options := s.options
...
func (s *Signer) PresignHTTP(
	ctx context.Context, credentials hmaccred.TPMSigner, r *http.Request,
